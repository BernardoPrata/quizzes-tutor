describe('Statistics testing', () => {
  ...
  it('tests if current course executions statistics data is currently presented in dashboard', () => {
    ...
    for (let year = 2020; year <=2022; year++) {
      ...
      cy.changeDemoStudentCourseExecutionMatchingAcademicTerm(`TEST- ${academicYear}`);
    }
    ...
    for (let year = 2020; year <= 2022; year++) {
        ...
        if (j > 0)
            cy.addQuizz(2,`Quizz 2 - ${academicYear}`,`TEST- ${academicYear}`,    date.toUTCString());
        if (j > 1)
            cy.addQuizz(3,`Quizz 3 - ${academicYear}`,`TEST- ${academicYear}`,    date.toUTCString());
        j++;
    }
    ...
    cy.selectCourseByTerm('TEST- Year 2021/2022');
    ...
    cy.get('.bar-chart canvas').eq(1).scrollIntoView().wait(5000).screenshot("./StudentStatsGraph")
